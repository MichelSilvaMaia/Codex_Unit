...
const returnInputSchema = z.object({
  purpose: z.literal("RETURN_EVIDENCE"), attachmentId: z.string().uuid(), clientOperationId: z.string().uuid(), deviceId: z.string().uuid(), tenantId: z.string().uuid(), userId: z.string().uuid(), aggregateId: z.string().uuid(), returnItemId: z.string().uuid(), expectedVersion: z.number().int().positive(), type: z.enum(["RETURN_CONDITION", "RETURN_DAMAGE", "MISSING_COMPONENT", "RESOURCE_IDENTIFICATION"]), checksum: z.string().regex(/^[a-f0-9]{64}$/),
});

export async function syncReturnAttachment(context: OperationalContext, raw: unknown, content: Uint8Array, mimeType: string, storage: StorageProvider = localStorageProvider) {
  ...
  requirePermission(context.permissions, "returns.add_evidence"); validateEvidence(content, mimeType);
  const checksum = createHash("sha256").update(content).digest("hex"); if (checksum !== input.checksum) throw new AppError("CONFLICT", "Checksum divergente.");
  ...
  if (!operation || operation.userId !== context.user.id || operation.deviceId !== input.deviceId || operation.aggregateType !== "ReservationReturn" || operation.aggregateId !== input.aggregateId || operation.operationType !== "RETURN_ATTACHMENTS") throw new AppError("FORBIDDEN", "Operação original não confirmada.");
  if (operation.resultVersion !== input.expectedVersion) throw new AppError("CONFLICT", "Versão incompatível com a inspeção.");
  const record = await prisma.reservationReturn.findFirst({ where: { id: input.aggregateId, tenantId: context.tenantId, status: "IN_PROGRESS", version: input.expectedVersion }, include: { items: { select: { id: true } } } });
  if (!record || !record.items.some(row => row.id === input.returnItemId)) throw new AppError("CONFLICT", "Devolução ou item alterado.");
  const identity = { tenantId: context.tenantId, attachmentId: input.attachmentId }, key = `${context.tenantId}/returns/${input.aggregateId}/offline-${input.attachmentId}`;
  let receipt = await prisma.offlineAttachmentReceipt.findUnique({ where: { tenantId_attachmentId: identity } });
  const same = (row: NonNullable<typeof receipt>) => { if (row.purpose !== input.purpose || row.userId !== context.user.id || row.deviceId !== input.deviceId || row.clientOperationId !== input.clientOperationId || row.aggregateId !== input.aggregateId || row.returnItemId !== input.returnItemId || row.expectedVersion !== input.expectedVersion || row.checksum !== checksum || row.mimeType !== mimeType || row.size !== content.byteLength || row.evidenceType !== input.type) throw new AppError("CONFLICT", "Identificador reutilizado com dados diferentes."); };
  if (receipt) { same(receipt); if (receipt.status === "CONFIRMED" && receipt.evidenceId) return { attachmentId: input.attachmentId, evidenceId: receipt.evidenceId, checksum, status: "SERVER_CONFIRMED" as const, duplicate: true }; if (receipt.claimedAt > expired()) throw busy(); const claimedAt = new Date(); if ((await prisma.offlineAttachmentReceipt.updateMany({ where: { id: receipt.id, status: "CLAIMED", claimedAt: { lte: expired() } }, data: { claimedAt } })).count !== 1) throw busy(); receipt = { ...receipt, claimedAt }; }
  else { try { receipt = await prisma.offlineAttachmentReceipt.create({ data: { ...identity, userId: context.user.id, deviceId: input.deviceId, clientOperationId: input.clientOperationId, aggregateId: input.aggregateId, returnItemId: input.returnItemId, expectedVersion: input.expectedVersion, purpose: input.purpose, checksum, mimeType, size: content.byteLength, evidenceType: input.type, status: "CLAIMED", storageKey: key } }); } catch (error) { if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") throw busy(); throw error; } }
  try { await storage.put(key, content, mimeType); const evidence = await prisma.$transaction(async tx => { const current = await tx.reservationReturn.findFirst({ where: { id: input.aggregateId, tenantId: context.tenantId, status: "IN_PROGRESS", version: input.expectedVersion } }); if (!current) throw new AppError("CONFLICT", "Devolução alterada durante upload."); const created = await tx.returnEvidence.create({ data: { tenantId: context.tenantId, returnId: input.aggregateId, returnItemId: input.returnItemId, type: input.type, storageKey: key, mimeType, size: content.byteLength, checksum, uploadedByUserId: context.user.id } }); if ((await tx.offlineAttachmentReceipt.updateMany({ where: { id: receipt!.id, status: "CLAIMED", claimedAt: receipt!.claimedAt }, data: { status: "CONFIRMED", evidenceId: created.id, confirmedAt: new Date() } })).count !== 1) throw busy(); return created; }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); return { attachmentId: input.attachmentId, evidenceId: evidence.id, checksum, status: "SERVER_CONFIRMED" as const, duplicate: false }; }
  catch (error) { const owned = await prisma.offlineAttachmentReceipt.findFirst({ where: { id: receipt!.id, status: "CLAIMED", claimedAt: receipt!.claimedAt } }); if (owned) { await storage.delete(key).catch(() => undefined); await prisma.offlineAttachmentReceipt.updateMany({ where: { id: receipt!.id, status: "CLAIMED", claimedAt: receipt!.claimedAt }, data: { claimedAt: new Date(0) } }); } throw error; }
}
