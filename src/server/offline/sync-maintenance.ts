...
export async function syncMaintenanceMutation(context: OperationalContext, raw: unknown) {
  ...
  requirePermission(context.permissions, input.operationType === "MAINTENANCE_ADD_ACTIVITY" ? "maintenance.perform" : "maintenance.diagnose");
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const result = input.operationType === "MAINTENANCE_ADD_ACTIVITY"
        ...
        : await tx.maintenanceDiagnosis.create({ data: { tenantId: context.tenantId, maintenanceOrderId: order.id, diagnosedByUserId: context.user.id, description: input.payload.description } });
