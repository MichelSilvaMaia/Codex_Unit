...
export function OfflinePickupDraft({ snapshot }: { snapshot: PickupSnapshot }) {
  ...
  const termsChanged = draft.recipientName !== record.recipientName || draft.recipientDocument !== record.recipientDocument || draft.recipientPhone !== record.recipientPhone || draft.vehiclePlate !== record.vehiclePlate || draft.notes !== record.notes || draft.items.some((item, i) => item.condition !== record.items[i]?.condition || item.notes !== record.items[i]?.notes);
  return <section className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-950">
    ...
    {termsChanged&&<p className="mt-3 text-sm font-semibold">O destinatário ou a inspeção mudou neste dispositivo. A assinatura fica bloqueada até os dados serem confirmados no servidor e os termos atualizados.</p>}
    {ready && <OfflinePickupAttachments snapshot={record} termsChanged={termsChanged}/>}
