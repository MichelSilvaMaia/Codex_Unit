...
export function OfflinePickupAttachments({ snapshot, terms, termsReady, unsavedChanges }: { snapshot: PickupSnapshot; terms: { version: string; hash: string; snapshot: string }; termsReady: boolean; unsavedChanges: boolean }) {
  ...
  async function prepare(blob: Blob, purpose: OfflineAttachment["purpose"], type: OfflineAttachment["type"], extra: Partial<OfflineAttachment> = {}) {
    if (localCompletion) throw new Error("Conclusão local já registrada; não adicione anexos depois dela.");
    if (!snapshot.draft || unsavedChanges || !termsReady) throw new Error("Salve a inspeção e aguarde a preparação dos termos antes de anexar.");
    ...
    if (!operation) {
      ...
      operation = { id, clientOperationId: id, tenantId: snapshot.tenantId, userId: snapshot.userId, deviceId: await offlineStore.deviceId(), operationType: "PICKUP_ATTACHMENTS", aggregateType: "ReservationPickup", aggregateId: snapshot.pickupId, expectedVersion: snapshot.expectedVersion, payload: { description: "Anexos de retirada", inspection: snapshot.draft }, dependsOnOperationIds: [], status: "PENDING", attemptCount: 0, createdAt: now, updatedAt: now, schemaVersion: 1 };
    }
    const attachment: OfflineAttachment = { id: crypto.randomUUID(), operationId: operation.id, tenantId: snapshot.tenantId, userId: snapshot.userId, aggregateType: "ReservationPickup", aggregateId: snapshot.pickupId, purpose, type, blob, mimeType: blob.type, size: blob.size, checksum: await digest(blob), expectedVersion: snapshot.expectedVersion + Number(Boolean(operation.payload.inspection)), createdAt: now, updatedAt: now, status: "LOCAL", attemptCount: 0, schemaVersion: 2, ...extra };
  ...
  async function saveSignature(blob: Blob, width: number, height: number) {
    if (busy) return;
    if (unsavedChanges || !termsReady || !snapshot.canSign) { setMessage("Salve o rascunho e aguarde os termos antes de assinar."); return; }
    ...
    try {
      ...
      await prepare(blob, "PICKUP_SIGNATURE", "SIGNATURE", { termsVersion: terms.version, termsHash: terms.hash, capturedAtDevice: new Date().toISOString(), width, height });
  ...
  async function completeLocally() {
    if (busy) return;
    setBusy(true);
    try { await offlineStore.queuePickupCompletion(snapshot.pickupId, snapshot.tenantId, snapshot.userId); await reload(); setMessage("Concluída neste dispositivo — aguardando sincronização. Recursos e custódia ainda não foram alterados no servidor."); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Não foi possível salvar a intenção de conclusão."); }
    finally { setBusy(false); }
  }

  const signature = attachments.find(row => row.purpose === "PICKUP_SIGNATURE");
  const localCompletion = operations.find(row => row.operationType === "PICKUP_COMPLETE");
  return <section className="mt-5 grid gap-4 border-t border-amber-300 pt-4">
    ...
    {snapshot.canAddEvidence && <form onSubmit={saveEvidence} className="grid gap-2 rounded-lg border bg-white p-3 sm:grid-cols-2">
      ...
      <button disabled={busy || unsavedChanges || !termsReady || Boolean(localCompletion)} className="rounded bg-amber-900 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Salvar foto neste dispositivo</button>
    </form>}
    ...
    <details className="rounded-lg border bg-white p-3 text-sm"><summary className="cursor-pointer font-semibold">Revisar termo antes de assinar · {terms.version}</summary><pre className="mt-2 whitespace-pre-wrap font-sans">{terms.snapshot}</pre><p className="mt-2 break-all text-xs">SHA-256: {terms.hash}</p></details>
    {snapshot.canSign && !signature && !unsavedChanges && termsReady && <SignaturePad pickupId={snapshot.pickupId} onOfflineSave={saveSignature}/>}
    {unsavedChanges && <p className="text-sm font-semibold text-red-800">Rascunho não salvo; assinatura offline bloqueada.</p>}
    ...
    {snapshot.canComplete && signature && !localCompletion && <button type="button" onClick={() => void completeLocally()} disabled={busy || unsavedChanges || !termsReady || snapshot.draft?.items.some(item => item.condition !== "OK")} className="w-fit rounded-lg bg-emerald-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Concluir neste dispositivo</button>}
    {localCompletion && <p className="rounded-lg border border-amber-400 bg-amber-100 p-3 text-sm font-semibold">{localCompletion.status === "CONFLICT" ? "Conclusão em conflito — revise sem descartar os dados" : localCompletion.status === "FAILED_PERMANENT" ? "Conclusão rejeitada — dados locais preservados" : "Concluída neste dispositivo — aguardando confirmação do servidor"}. Isto ainda não é saída/custódia definitiva.</p>}
