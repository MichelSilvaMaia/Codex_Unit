...
import { AttachmentBusyError, syncMaintenanceAttachment, syncPickupAttachment } from "@/server/offline/sync-attachment";
...
export async function POST(request: Request) {
  try {
    ...
    const result = metadata.purpose === "PICKUP_EVIDENCE" ? await syncPickupAttachment(context, metadata, bytes, file.type) : metadata.purpose === "PICKUP_SIGNATURE" ? await syncOfflineSignature(context, metadata, bytes, file.type) : await syncMaintenanceAttachment(context, metadata, bytes, file.type);
